...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n mycc -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/enccc_example >&log.txt
	res=$?
	cat log.txt
...
chaincodeQuery () {
  ...
  do
     ...
     peer chaincode query -C $CHANNEL_NAME -n mycc -c "{\"Args\":[\"ENC\",\"GET\",\"$2\"]}" --transient "{\"ENCKEY\":\"$ENCKEY\"}" >&log.txt
...
chaincodeInvoke () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		...
		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["ENC","PUT","key","vAlUe"]}' --transient "{\"ENCKEY\":\"$ENCKEY\"}" >&log.txt
	fi
	res=$?
	cat log.txt
...
chaincodeQueryIV () {
  ...
  do
     sleep 3
     echo "Attempting to Query PEER$PEER ...$(($(date +%s)-starttime)) secs"
     peer chaincode query -C $CHANNEL_NAME -n mycc -c "{\"Args\":[\"ENC\",\"GET\",\"$2\"]}" --transient "{\"ENCKEY\":\"$ENCKEY\",\"IV\":\"$IV\"}" >&log.txt
     test $? -eq 0 && VALUE=$(cat log.txt | awk '/Query Result/ {print $NF}') && let rc=0
    #  test "$VALUE" = "$2" && let rc=0
  done
  echo
 ...
 chaincodeInvokeIV () {
 	...
 	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
 		...
 		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["ENC","PUT","MyEncKey","MyValue"]}' --transient "{\"ENCKEY\":\"$ENCKEY\",\"IV\":\"$IV\"}" >&log.txt
 	fi
 	res=$?
 	cat log.txt
 	verifyResult $res "Invoke execution on PEER$PEER failed "
 ...
 chaincodeQuerySIG () {
   ...
   do
      sleep 3
      echo "Attempting to Query PEER$PEER ...$(($(date +%s)-starttime)) secs"
      peer chaincode query -C $CHANNEL_NAME -n mycc -c "{\"Args\":[\"SIG\",\"GET\",\"$2\"]}" --transient "{\"ENCKEY\":\"$ENCKEY\",\"SIGKEY\":\"$SIGKEY\"}" >&log.txt
      test $? -eq 0 && VALUE=$(cat log.txt | awk '/Query Result/ {print $NF}') && let rc=0
     #  test "$VALUE" = "$2" && let rc=0
   done
   echo
 ...
 chaincodeQuerySIG1 () {
 	...
 	do
 		 sleep 3
 		 echo "Attempting to Query PEER$PEER ...$(($(date +%s)-starttime)) secs"
 		 peer chaincode query -C $CHANNEL_NAME -n mycc -c '{"Args":["RANGE"]}' --transient "{\"ENCKEY\":\"$ENCKEY\"}" >&log.txt
 		 test $? -eq 0 && VALUE=$(cat log.txt | awk '/Query Result/ {print $NF}') && let rc=0
 		#  test "$VALUE" = "$2" && let rc=0
 	done
 	echo
  ...
  chaincodeInvokeSIG () {
  	...
  	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
  		...
  		peer chaincode invoke -o orderer.example.com:7050  --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -c '{"Args":["SIG","PUT","MySigKey","MyValue"]}' --transient "{\"ENCKEY\":\"$ENCKEY\",\"SIGKEY\":\"$SIGKEY\"}" >&log.txt
  	fi
  	res=$?
  	cat log.txt
  	verifyResult $res "Invoke execution on PEER$PEER failed "
...
chaincodeQuery 0 "key"

chaincodeInvokeIV 0

chaincodeQuery 0 "MyEncKey"

chaincodeQueryIV 0 "MyEncKey"

chaincodeInvokeSIG 0

chaincodeQuerySIG 0 "MySigKey"

chaincodeQuerySIG1 0 "MySigKey"
